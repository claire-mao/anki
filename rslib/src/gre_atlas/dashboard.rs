...
use crate::gre_atlas::topic_insights::select_recommended_topics;
use crate::gre_atlas::topic_insights::select_weak_topics;
...
impl Collection {
    pub fn gre_atlas_get_dashboard(&mut self, req: GetDashboardRequest) -> Result<DashboardState> {
        ...
        let signals = self.load_gre_atlas_signals(recent_limit)?;
        ...
        let recommended_topics =
            select_recommended_topics(&candidates, &weak_topics, insight_limit);

        Ok(DashboardState {
            memory: Some(signals.memory),
            performance: Some(signals.performance),
            readiness: Some(signals.readiness),
            coverage: Some(build_dashboard_coverage(&signals.coverage, &candidates)),
            weak_topics,
            recommended_topics,
...
mod test {
    ...
    fn get_dashboard_returns_state_for_empty_collection() -> Result<()> {
        ...
        let state = col.gre_atlas_get_dashboard(Default::default())?;
