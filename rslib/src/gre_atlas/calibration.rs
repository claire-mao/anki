...
use crate::gre_atlas::readiness::COVERAGE_WEIGHT;
use crate::gre_atlas::readiness::MEMORY_WEIGHT;
use crate::gre_atlas::readiness::PERFORMANCE_WEIGHT;
...
pub(crate) fn compute_calibration_stats(
    rows: &[ReadinessPredictionRow],
...
impl crate::collection::Collection {
    pub fn gre_atlas_get_readiness_calibration(&mut self) -> Result<ReadinessCalibrationResponse> {
...
fn calibration_curve(rows: &[&ReadinessPredictionRow]) -> Vec<ReadinessCalibrationBin> {
    ...
    for bin in 0..CALIBRATION_BIN_COUNT {
        ...
        let predicted_mean =
            in_bin.iter().map(|row| row.projected_score).sum::<f32>() / in_bin.len() as f32;
...
mod test {
    ...
    #[test]
    fn composite_readiness_score_uses_documented_weights() {
        let score = composite_readiness_score(80.0, 60.0, 0.5);
        let expected =
            (MEMORY_WEIGHT * 0.8 + PERFORMANCE_WEIGHT * 0.6 + COVERAGE_WEIGHT * 0.5) * 100.0;
    ...
    #[test]
    fn calibration_curve_bins_are_deterministic() {
        let rows: Vec<_> = (0..10)
            .map(|idx| {
                let id = (idx + 1) * 5;
                prediction(id, 50.0 + idx as f32 * 4.0, Some(48.0 + idx as f32 * 4.0))
            })
            .collect();
        let held_out: Vec<_> = rows.iter().collect();
        let first = calibration_curve(&held_out);
        let second = calibration_curve(&held_out);
        assert_eq!(first.len(), second.len());
        assert!(!first.is_empty());
        assert_eq!(first[0].bin_low, 50.0);
    }
