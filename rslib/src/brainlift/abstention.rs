...
fn fsrs_requirement(fsrs_enabled: bool) -> AbstentionRequirement {
    AbstentionRequirement {
        ...
        next_step: "Enable FSRS in deck options for the BrainLift GRE deck.".into(),
        met: fsrs_enabled,
    }
}
...
fn studied_cards_requirement(studied_cards: u32) -> AbstentionRequirement {
    ...
    AbstentionRequirement {
        ...
        next_step: format!(
            "Review cards in the BrainLift GRE deck until at least {MIN_STUDIED_CARDS} cards have one review."
        ),
        met,
    }
}
...
fn topic_coverage_requirement(coverage_ratio: f32) -> AbstentionRequirement {
    ...
    AbstentionRequirement {
        ...
        status: if met {
            ...
            format!("{current_pct}% of {required_pct}% exam-weighted catalog covered")
        },
        next_step: "Tag and review cards across more GRE topics to reach 50% catalog coverage.".into(),
        met,
    }
}
...
mod test {
    ...
    fn insufficient_topic_coverage_abstains() {
        ...
        assert!(reqs[2].status.contains("50%"));
    }
