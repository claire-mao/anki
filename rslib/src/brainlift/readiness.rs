...
use crate::brainlift::abstention::abstain_reason_from_requirements;
use crate::brainlift::abstention::memory_requirements;
use crate::brainlift::abstention::performance_requirements;
use crate::brainlift::abstention::readiness_requirements;
use crate::brainlift::abstention::sufficient_from_requirements;
...
pub(crate) struct MemoryInputs {
    ...
    pub topics: Vec<TopicMasteryEntry>,
}
        ...
        let covered_leaves = input
            ...
            .filter(|topic| {
                topic.studied_cards > 0
                    && crate::brainlift::GreCatalog::topic_by_id(&topic.topic_id)
...
pub(crate) fn compute_readiness_score(
    ...
    let requirements =
        readiness_requirements(&memory.abstention_requirements, &performance.abstention_requirements);
    let sufficient_data = sufficient_from_requirements(&requirements);
    ...
    let memory_width = memory.value_high.unwrap_or(projected) - memory.value_low.unwrap_or(projected);
    let perf_width =
        performance.value_high.unwrap_or(projected) - performance.value_low.unwrap_or(projected);
    let combined_margin =
        (memory_width * memory_width + perf_width * perf_width).sqrt() / 2.0;
...
mod test {
    use super::*;
    use crate::brainlift::abstention::REQ_PRACTICE_ATTEMPTS;
    use crate::brainlift::abstention::REQ_STUDIED_CARDS;
    use crate::brainlift::abstention::REQ_TOPIC_COVERAGE;
    use anki_proto::stats::TopicMasteryEntry;
    ...
    fn readiness_abstains_without_sufficient_inputs() {
        ...
        assert!(readiness.evidence_summary.contains("Performance insufficient"));
