...
impl BrainliftStorage {
    ...
    fn upgrade_from(&self, ver: u32) -> Result<()> {
        if ver < 2 {
            self.db
                .execute_batch(include_str!("upgrade_1_to_2.sql"))?;
        }
        if ver < 3 {
            self.db
                .execute_batch(include_str!("upgrade_2_to_3.sql"))?;
        }
        Ok(())
    }
    ...
    pub(crate) fn maybe_record_readiness_prediction(
        &self,
        snapshot: &crate::brainlift::calibration::ReadinessPredictionSnapshot,
    ) -> Result<()> {
        use crate::brainlift::calibration::MIN_PREDICTION_INTERVAL_SECS;
        use crate::brainlift::calibration::PREDICTION_SCORE_DELTA;
        use crate::brainlift::calibration::READINESS_MODEL_VERSION;
        ...
        if let Some(latest) = self.latest_readiness_prediction()? {
            ...
            if elapsed < MIN_PREDICTION_INTERVAL_SECS
                && (latest.projected_score - snapshot.projected_score).abs() < PREDICTION_SCORE_DELTA
            {
                return Ok(());
            }
        }
    ...
    pub(crate) fn resolve_pending_outcomes(
        &self,
        inputs: &crate::brainlift::calibration::OutcomeInputs,
    ) -> Result<()> {
        use crate::brainlift::calibration::resolve_outcome_score;
        ...
        for prediction in pending {
            ...
            let outcome_inputs = crate::brainlift::calibration::OutcomeInputs {
...
fn row_to_readiness_prediction(row: &rusqlite::Row<'_>) -> rusqlite::Result<ReadinessPredictionRow> {
...
mod test {
    ...
    fn open_v1_db(dir: &Path) -> Result<Connection> {
        let db_path = dir.join("brainlift.db");
        let db = Connection::open(db_path)?;
        db.execute_batch(
    ...
    fn list_questions_filters_by_topic_prefix() -> Result<()> {
        ...
        let storage = BrainliftStorage::open(&col_path)?;
