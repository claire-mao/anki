...
pub struct TopicDef {
    ...
    /// Official exam weight for leaf topics within their section (section leaves sum to 1.0).
    pub exam_weight: f32,
}
...
/// Canonical GRE topic catalog. Parent nodes organize the tree; leaves carry exam weights.
...
impl GreCatalog {
    ...
    /// Nearest catalog topic for an observed tag (exact id or longest prefix match).
...
mod test {
    ...
    fn topic_lookup_and_hierarchy() {
        ...
        assert!(children.iter().any(|t| t.id == "gre::quant::algebra::linear"));
    }
    ...
    fn nearest_topic_for_observed_tags() {
        ...
        assert_eq!(
            GreCatalog::nearest_topic_for_tag("gre::quant::algebra").unwrap().id,
            "gre::quant::algebra"
        );
