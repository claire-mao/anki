...
use crate::brainlift::signals::is_leaf_covered;
...
use crate::brainlift::TopicDef;
use crate::collection::Collection;
use crate::error::Result;
use crate::invalid_input;

impl Collection {
    pub fn brainlift_get_topic_details(
        &mut self,
        ...
        let topic_def = GreCatalog::topic_by_id(topic_id)
...
fn build_contribution_summary(
    ...
    if topic_def.is_leaf() {
        ...
        parts.push(
            "Parent topic — coverage and exam weight are rolled up from child topics".into(),
        );
    }
...
mod test {
    ...
    fn covered_leaf_has_coverage_contribution() {
        ...
        let expected =
            readiness::COVERAGE_WEIGHT * 100.0 * GreSection::QuantitativeReasoning.official_section_weight() * topic.exam_weight;
    ...
    fn memory_and_performance_contributions_scale_with_evidence() {
        ...
        assert!((performance_points - readiness::PERFORMANCE_WEIGHT * 100.0 * 0.25 * 0.8).abs() < 0.01);
