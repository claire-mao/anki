...
def handle_gre_atlas_bridge_cmd(mw: aqt.main.AnkiQt, cmd: str) -> bool:  # noqa: PLR0911
    ...
    if cmd == "greOpenFullStudy":
        open_gre_atlas(mw, path="dashboard")
        return True
...
def start_gre_review(mw: aqt.main.AnkiQt) -> None:
    ...
    mw.gre_review_pending_dashboard_refresh = True  # type: ignore[attr-defined]
    mw.col.startTimebox()
...
def refresh_gre_dashboard(mw: aqt.main.AnkiQt) -> None:
    ...
    if isinstance(dialog, GreAtlasDialog):
...
def open_gre_atlas(mw: aqt.main.AnkiQt, *, path: str = "dashboard") -> None:
    aqt.dialogs.open("GRE", mw, path=path)


class GreAtlasDialog(QDialog):
    silentlyClose = True
    ...
    def __init__(self, mw: aqt.main.AnkiQt, path: str = "dashboard") -> None:
        ...
        self.setWindowTitle("GRE Atlas")
        layout = QVBoxLayout(self)
        layout.setContentsMargins(0, 0, 0, 0)
        self.web = AnkiWebView(parent=self, kind=AnkiWebViewKind.GRE_ATLAS)
        layout.addWidget(self.web)
        restoreGeom(self, self.name, default_size=(1000, 800))
        self.web.set_bridge_command(
            lambda cmd: handle_gre_atlas_bridge_cmd(mw, cmd), self
        )
        self._load(path)
